...
interface TaskItem { id: string; text: string; status: 'open' | 'done'; runs?: TaskRun[] }
interface ChatMessage { role: 'user' | 'assistant'; content: string; ts: number }
interface StructuredMemory { project: string; decisions: string; preferences: string; tasks: { items: TaskItem[] } }
...
function readAgentConfig(folderPath: string): AgentConfig { return JSON.parse(readFileSync(getAgentConfigPath(folderPath), 'utf-8')) }
function writeAgentConfig(folderPath: string, config: AgentConfig) { writeFileSync(getAgentConfigPath(folderPath), JSON.stringify(config, null, 2), 'utf-8') }
function getMemoryPaths(folderPath: string) { const memoryDir = join(folderPath, AGENT_DIR, 'memory'); return { dir: memoryDir, project: join(memoryDir, 'project.md'), decisions: join(memoryDir, 'decisions.md'), preferences: join(memoryDir, 'preferences.md'), tasks: join(memoryDir, 'tasks.json'), chat: join(memoryDir, 'chat.json'), legacy: join(folderPath, AGENT_DIR, 'memory.md') } }
function withTaskIds(items: Array<{ id?: string; text: string; status: 'open' | 'done'; runs?: TaskRun[] }>): TaskItem[] { return items.map((item) => ({ id: item.id || `task-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, text: item.text, status: item.status, runs: item.runs || [] })) }
...
function updateTask(folderPath: string, taskId: string, updates: Partial<Pick<TaskItem, 'text' | 'status'>>) { const memory = loadStructuredMemory(folderPath); memory.tasks.items = memory.tasks.items.map(task => task.id === taskId ? { ...task, ...updates } : task); persistStructuredMemory(folderPath, memory); return memory.tasks.items }
function deleteTask(folderPath: string, taskId: string) { const memory = loadStructuredMemory(folderPath); memory.tasks.items = memory.tasks.items.filter(task => task.id !== taskId); persistStructuredMemory(folderPath, memory); return memory.tasks.items }
...
async function runTaskExecution(folderPath: string, taskId: string, event: Electron.IpcMainInvokeEvent) {
  ...
  try {
    ...
    if (refreshedTask) {
      ...
      refreshedTask.runs = (refreshedTask.runs || []).map(r => r.id === run.id ? { ...r, status: 'completed', completedAt: Date.now(), durationMs: Date.now() - r.startedAt, summary: finalResponse } : r)
...
function watchFolder(folderPath: string): void { if (activeWatcher) activeWatcher.close(); activeWatcher = chokidar.watch(folderPath, { ignored: /(^|[/\\])\.(foldermind|git)|node_modules|out|dist|release/, persistent: true, ignoreInitial: true }); activeWatcher.on('all', (event, filePath) => { pushFolderChange(folderPath, event, filePath); mainWindow?.webContents.send('folder:changed', { event, filePath }) }) }

ipcMain.handle('folder:create', async () => { if (!mainWindow) return null; const result = await dialog.showOpenDialog(mainWindow, { properties: ['openDirectory', 'createDirectory'], title: 'Choose or Create a Smart Folder', buttonLabel: 'Make it Smart' }); if (result.canceled || !result.filePaths.length) return null; const folderPath = result.filePaths[0]; const name = folderPath.split(/[\\/]/).pop() || 'Project'; await seedAgent(folderPath, name); watchFolder(folderPath); const structured = loadStructuredMemory(folderPath); return { path: folderPath, name, agentConfig: readAgentConfig(folderPath), memory: flattenStructuredMemory(structured) } })
...
ipcMain.handle('folder:updateConfig', async (_event, folderPath: string, updates: Partial<Pick<AgentConfig, 'tone' | 'archetype' | 'goals' | 'constraints'>>) => { const current = readAgentConfig(folderPath); const updated = { ...current, ...updates }; writeAgentConfig(folderPath, updated); return updated })
ipcMain.handle('tasks:list', async (_event, folderPath: string) => getTasks(folderPath))
ipcMain.handle('tasks:add', async (_event, folderPath: string, text: string) => addTask(folderPath, text))
